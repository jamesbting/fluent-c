...
{
	//chaeck if the variable defintion list is empty, if so allocate memory
	...
	{
		//non empty variable definitions - re allocate the memory for the new variable
		scope->variable_definitions_size += 1;
...
//iterate over all the variables within the scope to look for a variable with the same name
